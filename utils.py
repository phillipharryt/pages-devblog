...
def build_sidebar_soup(category_list: list[Category]) -> BeautifulSoup:
    """
    #### Inputs:
...
def add_sidebar_to_all_posts(sidebar_soup: BeautifulSoup) -> None:
    ...
    for category_dir in os.listdir('posts'):
        for file in os.listdir(f'posts/{category_dir}'):
            print('before')
            print(sidebar_soup)
            add_sidebar_to_html_file(f'posts/{category_dir}/{file}', sidebar_soup)
            print('after')
            print(sidebar_soup)
    add_sidebar_to_html_file('index.html', sidebar_soup)


def generate_category_page(category: Category) -> None:
    """
    #### Inputs:
    ...
    write_html(html_soup, f'posts/{category.name}/{category.name}.html')
...
def add_stylesheet_to_all_files():
    ...
    add_stylesheet_to_html_file('index.html', base_tufte)
    add_stylesheet_to_html_file('index.html', base_custom)
    for category in os.listdir('posts'):
        for file in os.listdir(f'posts/{category}'):
            add_stylesheet_to_html_file(f'posts/{category}/{file}', f'../../{base_tufte}')
            add_stylesheet_to_html_file(f'posts/{category}/{file}', f'../../{base_custom}')
            
            
def add_recent_posts_to_homepage():
    """
